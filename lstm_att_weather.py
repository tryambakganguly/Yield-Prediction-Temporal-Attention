...
# Model Summary
pred_model, prob_model = model(Tx, var_ts, h_s, dropout)
pred_model.summary()
...
def get_temporal_weights (x_data, dataset):
    
    y_data_hat_prob = prob_model.predict(x_data, batch_size = batch_size)
...
y_train_hat_alphas = get_temporal_weights(x_train, 'train')
y_val_hat_alphas = get_temporal_weights(x_val, 'val')
y_test_hat_alphas = get_temporal_weights(x_test, 'test')     